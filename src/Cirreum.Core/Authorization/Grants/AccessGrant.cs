...
/// <remarks>
/// <para>
/// <see cref="AccessGrant"/> is the Stage 1 gate's output and the handler's input. It answers:
...
/// <see cref="AccessGrantFactory"/> orchestrator produces it from an
/// app-provided <see cref="GrantResult"/>.
/// </para>
/// </remarks>
/// <param name="OwnerIds">
...
public sealed record AccessGrant(
	...
	/// </summary>
	public static AccessGrant Denied { get; } = new(OwnerIds: []);
	...
	/// </summary>
	public static AccessGrant Unrestricted { get; } = new(OwnerIds: null);
	...
	public static AccessGrant ForOwners(
		IReadOnlyList<string> ownerIds,
		IReadOnlyDictionary<string, object>? extensions = null) {
		...
		return ownerIds.Count == 0
			? Denied
			: new AccessGrant(ownerIds, extensions);
	}
