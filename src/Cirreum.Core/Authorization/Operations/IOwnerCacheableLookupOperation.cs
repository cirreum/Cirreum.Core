...
/// with <see cref="ICacheableOperation{TResultValue}"/> caching contract. The framework automatically
/// composes the final cache key as <c>{owner}:{boundary}:{CacheKey}</c> and adds a
/// <c>owner:{OwnerId}</c> tag via <see cref="Caching.CacheKeyContext"/>, which isolates
...
/// <see cref="ICacheableOperation{TResultValue}.CacheKey"/>, and shared state — <b>never</b> on the
/// caller's identity, roles, or per-caller authorization side-effects. Two callers resolving
/// the same <c>OwnerId</c> and <see cref="ICacheableOperation{TResultValue}.CacheKey"/> must always
/// get the same result.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// A <c>owner:{OwnerId}</c> tag is automatically added to
/// <see cref="ICacheableOperation{TResultValue}.CacheTags"/>, enabling bulk invalidation of a
...
public interface IOwnerCacheableLookupOperation<TResultValue>
	: IOwnerLookupOperation<TResultValue>, ICacheableOperation<TResultValue>;